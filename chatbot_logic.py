...
def generate_bot_reply(api_key: str, message: str, is_first_in_session: bool = False, 
    ...
    """Генерация ответа бота через Replicate API с учетом контекста процедуры."""
    try:
        print(f"\n🤖 Генерация ответа AI")
...
"""
            # Добавляем контекст процедуры если есть
            procedure_context = ""
            if last_procedure:
                procedure_context = f"\nКОНТЕКСТ: Клиент ранее интересовался процедурой '{last_procedure}'."
            
            full_prompt = f"""{system_prompt}{procedure_context}
...
ОТВЕТ:"""
            ...
            context_lines = []
            context_lines.append(f"- {'Это начало диалога, нужно представиться' if is_first_in_session else 'Диалог уже идет, не представляйся снова'}")
            context_lines.append(f"- Клиент хочет записаться: {wants_registration}")
            context_lines.append(f"- {'У клиента уже есть имя и телефон' if has_name and has_phone else 'Контактов еще нет'}")
            context_lines.append(f"- {'ЗАЯВКА УЖЕ ОТПРАВЛЕНА' if telegram_sent else 'Заявка еще не отправлена'}")
            
            # Добавляем контекст процедуры если есть
            if last_procedure:
                context_lines.append(f"- Ранее обсуждалась процедура: {last_procedure}")
                context_lines.append(f"- Если клиент хочет записаться без уточнения процедуры, СПРОСИ: 'На какую именно процедуру хотите записаться?'")
            
            context_section = "\n".join(context_lines)
            
            # Особые инструкции если есть контекст процедуры
            special_instructions = ""
            if last_procedure:
                special_instructions = """
ВАЖНО КОНТЕКСТА:
- Если клиент хочет записаться, но не указал процедуру — уточни: 'На какую именно процедуру хотите записаться? Вы ранее интересовались [процедура]'
- Если клиент спрашивает о процедуре, которая обсуждалась ранее — дай подробную информацию с ценами
- Используй контекст для более персонализированных ответов
"""
...
5. Если клиент ХОЧЕТ ЗАПИСАТЬСЯ:
   - Если он уже указал процедуру — попроси имя и телефон
   - Если НЕ указал процедуру — уточни какую процедуру он хочет
...
8. Учитывай контекст предыдущих сообщений
9. Будь экспертом, но дружелюбной
...
ОТВЕТ:"""
        ...
        if telegram_sent:
            ...
            # Используем контекст процедуры если есть
            if last_procedure:
