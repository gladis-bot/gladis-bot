import replicate
import re
import json
import os

# Загружаем прайс из файла
def load_procedures_prices():
    """Загружает полный прайс из файла."""
    try:
        file_path = os.path.join(os.path.dirname(__file__), 'data', 'procedures.json')
        with open(file_path, 'r', encoding='utf-8') as f:
            return json.load(f)
    except Exception as e:
        print(f"❌ Ошибка загрузки прайса: {e}")
        return {}

# Создаем динамический SYSTEM_PROMPT с полным прайсом
def create_system_prompt():
    """Создает SYSTEM_PROMPT с актуальным прайсом."""
    procedures_data = load_procedures_prices()
    
    base_prompt = """
Ты — Александра, менеджер клиники эстетической медицины GLADIS в Сочи.

ТВОЙ СТИЛЬ ОБЩЕНИЯ:
- Дружелюбная, профессиональная, эксперт по всем процедурам клиники
- Отвечай на вопросы ПОЛНО и ИНФОРМАТИВНО
- Если спрашивают о процедуре — расскажи что это, зачем делают, сколько стоит
- Если хотят записаться — попроси имя и телефон
- Будь полезной и конкретной

ВАЖНЫЕ ПРАВИЛА:
1. ПРЕДСТАВЛЯЙСЯ ТОЛЬКО ПРИ ПЕРВОМ ОТВЕТЕ ИЛИ ЕСЛИ КЛИЕНТ СПРАШИВАЕТ "КТО ТЫ?"
2. НЕ ПОВТОРЯЙ "ЗДРАВСТВУЙТЕ" В КАЖДОМ СООБЩЕНИИ
3. НЕ добавляй контакты к простым информационным ответам
4. Контакты добавляй ТОЛЬКО если:
   - Клиент явно говорит "хочу записаться", "запишите меня", "давайте запишемся"
   - Клиент спрашивает "как записаться?"
   - Завершаешь консультацию и клиент готов к записи

КОНТАКТЫ КЛИНИКИ (добавляй только когда нужно):
📞 Телефон: 8-928-458-32-88
📍 Адреса:
   • Сочи: ул. Воровского, 22
   • Адлер: ул. Кирова, д. 26а
⏰ Работаем ежедневно с 10:00 до 20:00

ВАЖНО: Рассрочку и кредит НЕ предоставляем!
"""

    # Добавляем полный прайс
    price_section = "\nПОЛНЫЙ ПРАЙС ПРОЦЕДУР КЛИНИКИ GLADIS:\n\n"
    
    if procedures_data and 'procedures' in procedures_data:
        for procedure in procedures_data['procedures']:
            name = procedure.get('name', '')
            category = procedure.get('category', '')
            
            price_section += f"{name} ({category}):\n"
            
            # Добавляем цены если есть
            if 'prices' in procedure and procedure['prices']:
                for zone, price in procedure['prices'].items():
                    price_section += f"  • {zone}: {price} руб.\n"
            
            # Добавляем комплексы если есть
            if 'complexes' in procedure and procedure['complexes']:
                price_section += f"  Комплексы:\n"
                for complex_name, complex_price in procedure['complexes'].items():
                    price_section += f"    • {complex_name}: {complex_price} руб.\n"
            
            # Добавляем специальные заметки
            if 'note' in procedure:
                price_section += f"  Примечание: {procedure['note']}\n"
            
            price_section += "\n"
    
    # Если не удалось загрузить прайс, добавляем основные процедуры
    else:
        price_section += """
Основные процедуры и цены:

1. ЛАЗЕРНАЯ ЭПИЛЯЦИЯ:
   • Подмышки: 1100-1400 руб
   • Бикини: 1900-3500 руб
   • Голени: 2550-3300 руб
   • Ноги полностью: 4500-5800 руб

2. БИОРЕВИТАЛИЗАЦИЯ:
   • Коктейль Monaco: 18000-25000 руб
   • Hyaron: 5500 руб
   • Profhilo: 18000 руб
   • Трихолакс (для волос): 6000 руб

3. ЧИСТКА ЛИЦА:
   • Ультразвуковая/механическая: 3900 руб
   • Карбоновый пилинг: 3350 руб
   • Hydrafacial: 4500 руб

4. БОТУЛОТОКСИН:
   • Релатокс/Ботулакс: 250 руб/ед.
   • Гипергидроз: 20000 руб

5. ПИЛИНГИ: от 2000 до 6000 руб
6. ЛИФТИНГ: от 16500 до 60000 руб
7. ПЕРМАНЕНТНЫЙ МАКИЯЖ: от 5500 до 8000 руб
"""
    
    full_prompt = base_prompt + price_section
    
    return full_prompt

def is_simple_greeting(message: str) -> bool:
    """Проверяет, является ли сообщение простым приветствием."""
    message_lower = message.lower()
    
    greetings = [
        "добрый день", "добрый вечер", "доброе утро",
        "здравствуйте", "привет", "здрасьте", "приветствую",
        "доброго времени суток", "доброй ночи", "добрый",
        "здравия", "приветик", "доброго"
    ]
    
    # Если сообщение состоит только из приветствия
    for greeting in greetings:
        if greeting in message_lower:
            clean_msg = message_lower.replace(greeting, "").strip()
            if not clean_msg or len(clean_msg.replace(" ", "")) < 3:
                return True
    
    return False

def is_registration_request(message: str) -> bool:
    """Проверяет, хочет ли клиент записаться."""
    message_lower = message.lower()
    
    # Исправляем опечатки
    corrected_message = message_lower
    corrections = {
        "хочцу": "хочу",
        "записацца": "записаться", 
        "записатся": "записаться",
        "записаца": "записаться",
        "записацо": "записаться"
    }
    
    for wrong, correct in corrections.items():
        corrected_message = corrected_message.replace(wrong, correct)
    
    registration_phrases = [
        "хочу записаться", "запишите меня", "можно записаться",
        "готов записаться", "давайте запишем", "хочу на процедуру",
        "хотел бы записаться", "запишите пожалуйста", "запись на",
        "как записаться", "можно записаться на", "хочу сделать запись",
        "хочу попасть на", "мне нужно записаться", "нужна запись",
        "хочу записаться на", "записать на"
    ]
    
    # Проверяем исправленное сообщение
    if any(phrase in corrected_message for phrase in registration_phrases):
        return True
    
    # Сообщения содержащие "запис" и глаголы действия
    if "запис" in corrected_message:
        action_words = ["хочу", "можно", "нужно", "готов", "давайте", "пожалуйста", "мне"]
        if any(word in corrected_message for word in action_words):
            return True
    
    return False

def should_add_contacts_to_reply(user_message: str, bot_reply: str, is_first_message: bool = False) -> bool:
    """
    Определяет, нужно ли добавлять контакты к ответу.
    """
    user_lower = user_message.lower()
    reply_lower = bot_reply.lower()
    
    # Всегда добавляем контакты если:
    # 1. Клиент явно хочет записаться
    if is_registration_request(user_message):
        return True
    
    # 2. Клиент спрашивает контакты
    if any(word in user_lower for word in ["телефон", "адрес", "контакт", "позвонить", "номер", "как связаться"]):
        return True
    
    # 3. В ответе уже просят контакты (явно)
    if any(phrase in reply_lower for phrase in [
        "для записи мне нужно ваше имя и телефон",
        "укажите ваше имя и телефон для записи",
        "назовите имя и телефон для связи",
        "мне нужны ваши контакты для записи"
    ]):
        return True
    
    # 4. Это завершение консультации (длинный ответ) И клиент проявлял интерес
    if len(bot_reply) > 300 and ("руб" in reply_lower or "стоимость" in reply_lower):
        # Проверяем, было ли в диалоге упоминание о записи
        if "запис" in user_lower:
            return True
    
    # Не добавляем контакты если:
    # 1. Это просто приветствие
    if is_simple_greeting(user_message):
        return False
    
    # 2. Это простой информационный вопрос
    if (len(bot_reply) < 200 and 
        "?" in user_message and 
        not is_registration_request(user_message)):
        return False
    
    return False

def generate_bot_reply(api_key: str, message: str, is_first_in_session: bool = False, 
                      has_name: bool = False, has_phone: bool = False) -> str:
    """Генерация ответа бота через Replicate API."""
    try:
        print(f"\n🤖 Генерация ответа AI")
        print(f"   Сообщение: '{message}'")
        print(f"   Первое в сессии: {is_first_in_session}")
        print(f"   Есть имя: {has_name}, Есть телефон: {has_phone}")
        
        message_lower = message.lower()
        
        # 1. Очень простые случаи обрабатываем сразу
        if is_simple_greeting(message_lower):
            if is_first_in_session:
                return "Здравствуйте! Клиника GLADIS, меня зовут Александра. Чем могу вам помочь?"
            else:
                return "Чем могу вам помочь?"
        
        # 2. Проверяем, хочет ли клиент записаться
        wants_registration = is_registration_request(message)
        
        # 3. Если уже есть контакты, клиент хочет записаться, но еще не отправили в Telegram
        if has_name and has_phone and wants_registration:
            # Клиент дал контакты и хочет записаться
            system_prompt = """
Ты — Александра, менеджер клиники эстетической медицины GLADIS в Сочи.
Клиент уже предоставил имя и телефон, хочет записаться на процедуру.
Твоя задача — подтвердить получение заявки и сообщить что она передана менеджеру.
Не проси дополнительные контакты, не задавай лишних вопросов.
Будь вежливой и профессиональной.
"""
            full_prompt = f"""{system_prompt}

Клиент уже дал контакты (имя и телефон).
Клиент хочет записаться на процедуру.

КЛИЕНТ пишет: "{message}"

Твой ответ (коротко и по делу):
1. Подтверди что получили заявку
2. Скажи что передали менеджеру
3. Упомяни что свяжутся для подтверждения
4. Будь вежливой

ОТВЕТ:"""
        
        else:
            # Обычный диалог
            system_prompt = create_system_prompt()
            
            full_prompt = f"""{system_prompt}

КОНТЕКСТ ДИАЛОГА:
- {'Это начало диалога, нужно представиться' if is_first_in_session else 'Диалог уже идет, не представляйся снова'}
- Клиент хочет записаться: {wants_registration}
- {'У клиента уже есть имя и телефон' if has_name and has_phone else 'Контактов еще нет'}

КЛИЕНТ пишет: "{message}"

Твой ответ (как Александра):
1. {'Представься: "Здравствуйте! Клиника GLADIS, меня зовут Александра."' if is_first_in_session else 'НЕ представляйся снова'}
2. Ответь ПОЛНО на вопрос
3. Если клиент ХОЧЕТ ЗАПИСАТЬСЯ и нет контактов — попроси имя и телефон
4. Если уже есть контакты — подтверди получение
5. Если это просто информационный вопрос — просто ответь
6. Будь экспертом, но дружелюбной

ОТВЕТ:"""
        
        # 4. Используем AI
        client = replicate.Client(api_token=api_key)
        
        output = replicate.run(
            "meta/meta-llama-3-70b-instruct",
            input={
                "prompt": full_prompt,
                "max_tokens": 800,
                "temperature": 0.7,
                "top_p": 0.9
            }
        )
        
        # 5. Обрабатываем ответ
        result = ""
        if hasattr(output, '__iter__') and not isinstance(output, str):
            for chunk in output:
                if isinstance(chunk, str):
                    result += chunk
                else:
                    result += str(chunk)
        elif isinstance(output, str):
            result = output
        else:
            result = str(output)
        
        result = result.strip()
        print(f"   Ответ AI (сырой): '{result[:200]}...'")
        
        # 6. Очищаем ответ если нужно
        if not result or len(result) < 10:
            result = "Извините, не удалось обработать запрос. Пожалуйста, позвоните нам по телефону 8-928-458-32-88 для консультации."
        
        # 7. Убираем повторные приветствия если не первое сообщение
        if not is_first_in_session:
            result = re.sub(r'^Здравствуйте[!\.]?\s*', '', result)
            result = re.sub(r'^Добрый день[!\.]?\s*', '', result)
            result = re.sub(r'^Добрый вечер[!\.]?\s*', '', result)
            result = re.sub(r'^Привет[!\.]?\s*', '', result)
            result = re.sub(r'^Меня зовут Александра[!\.]?\s*', '', result)
        
        # 8. Если уже есть контакты и клиент хочет записаться, добавляем завершающую фразу
        if has_name and has_phone and wants_registration:
            if "спасибо" not in result.lower() and "переда" not in result.lower():
                result += "\n\nЗаявка передана менеджеру. С вами свяжутся для подтверждения записи!"
        
        return result
            
    except Exception as e:
        print(f"❌ Ошибка AI: {str(e)}")
        import traceback
        print(f"❌ Traceback: {traceback.format_exc()}")
        
        # Fallback на случай ошибки AI
        message_lower = message.lower()
        
        if has_name and has_phone:
            return "✅ Спасибо! Ваша заявка передана менеджеру. С вами свяжутся для подтверждения записи."
        elif "запис" in message_lower:
            return "Для записи укажите ваше имя и телефон. Телефон клиники: 8-928-458-32-88"
        elif "адрес" in message_lower or "телефон" in message_lower:
            return "Клиника GLADIS:\n📞 Телефон: 8-928-458-32-88\n📍 Адреса: Сочи, ул. Воровского, 22 и Адлер, ул. Кирова, д. 26а"
        else:
            return "Для консультации по процедурам позвоните по телефону 8-928-458-32-88"

def extract_name_with_ai(api_key: str, message: str) -> str:
    """
    Использует AI для извлечения имени человека из сообщения.
    Возвращает только человеческие имена, отфильтровывает названия процедур.
    """
    try:
        prompt = f"""Определи, есть ли в сообщении имя человека. Если есть - верни ТОЛЬКО имя. Если нет - верни "not_found".

ВОТ ПРАВИЛА:
1. Имя - это личное имя человека (Анна, Иван, Мария, Дмитрий и т.д.)
2. НЕ имя: названия процедур (ботокс, эпиляция, чистка, пилинг и т.д.)
3. НЕ имя: общие слова (привет, здравствуйте, хочу, записаться и т.д.)
4. Если сомневаешься - верни "not_found"

Примеры:
Сообщение: "Меня зовут Анна, хочу на ботокс" → Ответ: "Анна"
Сообщение: "Хочу записаться на ботокс" → Ответ: "not_found"
Сообщение: "Иван, 89161234567" → Ответ: "Иван"
Сообщение: "Ботокс на губы" → Ответ: "not_found"

Сообщение: "{message}"

Ответ (только имя или "not_found"):"""

        client = replicate.Client(api_token=api_key)
        
        output = client.run(
            "meta/meta-llama-3-70b-instruct",
            input={
                "prompt": prompt,
                "max_tokens": 20,
                "temperature": 0.1,
                "top_p": 0.9
            }
        )
        
        # Обрабатываем ответ
        result = ""
        if hasattr(output, '__iter__') and not isinstance(output, str):
            for chunk in output:
                if isinstance(chunk, str):
                    result += chunk
                else:
                    result += str(chunk)
        elif isinstance(output, str):
            result = output
        else:
            result = str(output)
        
        result = result.strip().lower()
        print(f"🔍 AI анализ имени из '{message}': получил '{result}'")
        
        # Очищаем ответ
        if result in ['not_found', 'none', 'null', 'нет', 'no name', '']:
            return None
        
        # Удаляем кавычки и лишние символы
        result = re.sub(r'["\'\.,!?]', '', result).strip()
        
        if not result:
            return None
        
        # Фильтруем процедуры (последняя проверка)
        procedure_keywords = [
            'ботокс', 'ботулин', 'диспорт', 'релатокс', 'ботулакс',
            'эпиляция', 'лазерная', 'биоревитализация', 'чистка',
            'пилинг', 'лифтинг', 'смас', 'морфиус', 'перманентный',
            'макияж', 'контурная', 'пластика', 'мезотерапия'
        ]
        
        if any(proc in result for proc in procedure_keywords):
            print(f"⚠️ Отфильтровано: '{result}' похоже на процедуру")
            return None
        
        # Проверяем что это похоже на имя (только буквы и дефисы)
        if not re.match(r'^[А-ЯЁа-яё\-]+$', result):
            return None
        
        # Капитализируем первую букву
        if '-' in result:
            parts = result.split('-')
            result = '-'.join([part.capitalize() for part in parts])
        else:
            result = result.capitalize()
        
        # Длина имени должна быть разумной
        if len(result) < 2 or len(result) > 30:
            return None
        
        return result if result else None
            
    except Exception as e:
        print(f"❌ Ошибка AI при извлечении имени: {str(e)}")
        return None

def check_interesting_application(text: str):
    """
    Проверяем, является ли сообщение заявкой на процедуру.
    """
    t = text.lower()
    
    # Ключевые слова для определения заявки
    procedure_keywords = [
        "записаться", "запись", "записать", "хочу на", "можно записаться",
        "запишите", "запишите меня", "запишите на", "запись на",
        "сколько стоит", "цена", "стоимость", "прайс",
        "эпиляция", "лазерная", "ботокс", "ботулин", "чистка",
        "пилинг", "омоложение", "лифтинг", "smas", "морфиус",
        "биоревитализация", "инъекция", "укол", "гиалуроновая",
        "консультация", "врач", "косметолог", "прием",
        "брови", "ресницы", "ламинирование", "наращивание",
        "тату", "татуировка", "удаление", "перманент",
        "массаж", "микротоки", "мезотерапия"
    ]
    
    # Если есть слова "имя" или "телефон" - это точно заявка
    contact_keywords = ["имя", "зовут", "телефон", "телефоне", "номер", "позвонить", "мне зовут"]
    
    has_procedure = any(keyword in t for keyword in procedure_keywords)
    has_contacts = any(keyword in t for keyword in contact_keywords)
    
    print(f"🔍 Проверка заявки: '{text[:50]}...'")
    print(f"   Процедурные слова: {has_procedure}")
    print(f"   Контактные слова: {has_contacts}")
    
    # Заявкой считаем если:
    # 1. Есть слова о процедурах/ценах ИЛИ
    # 2. Клиент явно указывает контакты
    return has_procedure or has_contacts
