...
def generate_bot_reply(api_key: str, message: str, is_first_in_session: bool = False, 
    ...
    try:
        ...
        if telegram_sent:
            if has_name:
                return f"✅ {has_name}, ваша заявка передана менеджеру. С вами свяжутся для подтверждения записи.\n\n📞 Телефон клиники: 8-928-458-32-88"
            else:
                return "✅ Ваша заявка передана менеджеру. С вами свяжутся для подтверждения записи.\n\n📞 Телефон клиники: 8-928-458-32-88"
        
        # 3. ТОЛЬКО явные специфичные случаи обрабатываем отдельно
        # 3.1 Вопросы про пигментацию (очень специфично)
        ...
        # 3.2 Проверяем, явный ли это запрос на запись (БАЗОВАЯ проверка)
        basic_registration_check = is_registration_request(message)
        
        # 3.3 Вопросы про аппараты - ТОЛЬКО если это точно не запрос и явный вопрос
        ...
        context_lines.append(f"   • {'🚨 Заявка уже отправлена в Telegram' if telegram_sent else '📝 Заявка еще не отправлена'}")
...
ОТВЕТ:"""
        ...
        # Автокоррекция: если AI забыл попросить контакты при явной записи
        if basic_registration_check and "запис" in message_lower:
