...
def generate_bot_reply(api_key: str, message: str) -> str:
    ...
    try:
        ...
        if is_simple_greeting(message_lower) and not _first_greeting_done:
            _first_greeting_done = True
            return "Здравствуйте! Клиника GLADIS, меня зовут Александра. Чем могу вам помочь?"
        elif is_simple_greeting(message_lower) and _first_greeting_done:
            # Уже поздоровались, просто отвечаем
            return "Чем могу вам помочь?"
        
        # 2. Проверяем, хочет ли клиент записаться
        wants_registration = is_registration_request(message)
        
        # 3. Создаем динамический промпт с актуальным прайсом
        system_prompt = create_system_prompt()
        
        # 4. Формируем полный промпт для AI
        greeting_part = "Здравствуйте! " if not _first_greeting_done else ""
        _first_greeting_done = True
        
        full_prompt = f"""{system_prompt}

КОНТЕКСТ ДИАЛОГА:
- Клиент уже {'поздоровался' if _first_greeting_done else 'еще не здоровался'}
- {'Уже представились как Александра' if _first_greeting_done else 'Нужно представиться'}
- Клиент хочет записаться: {wants_registration}

КЛИЕНТ пишет: "{message}"

Твой ответ (как Александра):
1. {'НЕ представляйся снова, уже знакомы' if _first_greeting_done else 'Представься: "Здравствуйте! Клиника GLADIS, меня зовут Александра."'}
2. Ответь ПОЛНО на вопрос
3. Если спрашивают о процедуре — расскажи что это, для чего, сколько стоит
4. Если клиент ХОЧЕТ ЗАПИСАТЬСЯ ({wants_registration}) — попроси имя и телефон
5. Если это просто информационный вопрос — НЕ проси контакты
...
ОТВЕТ:"""
        ...
        # 8. Убираем повторные приветствия из ответа AI
        if _first_greeting_done:
            # Убираем "Здравствуйте!" если это не первое сообщение
        ...
        # 9. Проверяем, нужно ли добавить контакты
        if should_add_contacts_to_reply(message, result, not _first_greeting_done):
