...
def is_registration_request(message: str) -> bool:
    """Проверяет, хочет ли клиент записаться. БАЗОВАЯ проверка."""
    message_lower = message.lower()
    
    # Только самые явные признаки
    if "запис" in message_lower and any(word in message_lower for word in ["хочу", "можно", "нужно", "готов"]):
        return True
    
    # Или явное указание времени
    if any(word in message_lower for word in ["завтра", "сегодня", "в понедельник", "после"]) and "запис" in message_lower:
        return True
...
def generate_bot_reply(api_key: str, message: str, is_first_in_session: bool = False, 
    ...
    try:
        ...
        # 3. Проверяем специальные вопросы (пигментация, аппараты) - ТОЛЬКО если это не запрос на запись
        # Сначала базовая проверка на запись
        basic_registration_check = is_registration_request(message)
        
        if not basic_registration_check:
            # Только если не похоже на запись, проверяем специальные вопросы
        ...
        # 4. ВСЕ остальное отдаем AI, он сам разберется
...
Сообщение: "{message}"
{'- Похоже на запрос на запись' if basic_registration_check else '- Похоже на информационный вопрос'}

ВАЖНО: Ты сама определи намерение клиента и действуй соответственно:
1. Если клиент ХОЧЕТ ЗАПИСАТЬСЯ:
   - Попроси имя и телефон
   - Если процедура не указана, но есть контекст — уточни
   - Если процедура указана — подтверди детали и попроси контакты

2. Если клиент СПРАШИВАЕТ ИНФОРМАЦИЮ:
   - Дай полный ответ из прайса
   - Упомяни аппараты если спрашивают
   - Предложи записаться если уместно
...
2. Определи намерение клиента (запись или информация)
3. Действуй соответственно
