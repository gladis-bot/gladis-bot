...
1. Клиент ХОЧЕТ записаться если:
   - Явно говорит "хочу записаться", "запишите меня", "хочу сделать", "можно записаться"
   - Спрашивает цену И потом дает контакты (имя и телефон)
   - Интересуется конкретной процедурой И дает свои данные
   - Просит записать на конкретную процедуру или зону
   - Соглашается на предложение записаться
   - Говорит "хочу", "нужно", "готов" в контексте процедуры
   - Прошел консультацию и теперь предоставляет контакты

2. Клиент НЕ хочет записаться если:
   - Только спрашивает информацию без намерения записаться
   - Просто уточняет детали без контактов
   - Обсуждает в общем, без конкретных планов
   - Только интересуется ценами без дальнейших действий
   - Просто представился без упоминания процедур
...
ОТВЕТ (ТОЛЬКО ОДНО СЛОВО: "ДА" или "НЕТ"):"""
        ...
        # Проверяем ответ
        if "да" in result or "yes" in result or "true" in result:
            return True
        elif "нет" in result or "no" in result or "false" in result:
            return False
        else:
            # Если AI дал неоднозначный ответ, проверяем по контексту
            # Клиент хочет записаться если в сообщении есть слова о действии
            action_words = ["запис", "хочу", "нужно", "можно", "готов", "давайте", "хотел", "интересует"]
            return any(word in current_message.lower() for word in action_words)
