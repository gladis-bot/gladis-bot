...
def analyze_client_needs(message: str, session: Dict[str, Any]) -> str:
    ...
    procedures_info = {
        "эпиляция": {
            ...
            "description": "Удаление волос лазером - долговременный результат после курса процедур",
            "types": [
                "Гибридный лазер Innovation - более мягкий, больше сеансов (8-12)",
                "Александритовый лазер Quanta System - более мощный, меньше сеансов (5-7)"
            ],
            "questions": [
                "Какие зоны интересуют? (подмышки, бикини, ноги, лицо и т.д.)",
                "Какой у вас тип кожи и цвет волос? (светлые/темные, тонкие/грубые)"
            ]
        },
        "пилинг": {
            "name": "Пилинг",
    ...
    # Сначала проверяем явное упоминание эпиляции
    if any(word in message_lower for word in ["эпиляция", "лазер", "удаление волос", "подмышек", "бикини", "голени", "ноги"]):
        found_procedure = "эпиляция"
        session['procedure_category'] = "Лазерная эпиляция"
    else:
        # Проверяем другие процедуры
    ...
    if found_procedure:
        ...
        if found_procedure == "эпиляция":
            ...
            response += f"У нас есть следующие варианты:\n"
        ...
        response += "Поняла, вы спрашиваете про консультацию. Расскажите подробнее, что именно вас интересует?\n\n"
        response += "Например:\n"
        response += "- Лазерная эпиляция (удаление волос)\n"
        response += "- Чистка или пилинг для лица\n"
        response += "- Ботулотоксин от морщин\n"
        response += "- Лифтинг или подтяжка кожи\n\n"
        response += "Или задайте конкретный вопрос о процедурах!"
    
    # Всегда добавляем контакты в конце диалога, а не сразу
